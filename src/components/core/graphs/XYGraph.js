...
const XYGraph = ({
  ...
  const minX =
    hasData && isDate(data[0].x)
      ? new Date(Math.min(...data.map((point) => new Date(point.x))))
      : 0;

  const maxX =
    hasData && isDate(data[0].x)
      ? new Date(Math.max(...data.map((point) => new Date(point.x))))
      : hasData
      ...
      <div className="relative">
        <svg width={width + padding} height={height + padding}>
          ...
          {Array.from({ length: gridLinesCount }).map((_, i) => (
            <line
              key={i}
              x1={((width - 2 * padding) / gridLinesCount) * (i + 1) + padding}
              y1={padding}
              x2={((width - 2 * padding) / gridLinesCount) * (i + 1) + padding}
              y2={height - padding}
              stroke="#e0e0e0"
              strokeWidth="1"
            />
          ...
          {Array.from({ length: gridLinesCount }).map((_, i) => (
            <line
              key={i}
              x1={padding}
              y1={((height - 2 * padding) / gridLinesCount) * (i + 1) + padding}
              x2={width - padding}
              y2={((height - 2 * padding) / gridLinesCount) * (i + 1) + padding}
              stroke="#e0e0e0"
              strokeWidth="1"
            />
          ))}
            ...
            data.map((point, index) => (
              <text
                ...
                x={
                  padding +
                  (new Date(point.x) -
                    new Date(Math.min(...data.map((p) => new Date(p.x))))) *
                    scaleX
                }
                y={height - padding + 20}
                ...
                transform={`rotate(-45, ${
                  ...
                  (new Date(point.x) -
                    new Date(Math.min(...data.map((p) => new Date(p.x))))) *
                    scaleX
                }, ${height - padding + 20})`} // Rotate by -45 degrees
              >
                {new Date(point.x).toLocaleDateString()}
              </text>
            ))}
